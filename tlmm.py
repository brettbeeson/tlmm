...
class TLMovieMaker:
    ...
    def __str__(self):
        return "{}: Movies:{}  SpeedUp:{} DaySliceLength:{} fileList:{} motion:{}".format(type(self).__name__,
                                                                                          len(self.tlMovies),
                                                                                          self.speedup,
                                                                                          self.daySliceLength,
                                                                                          len(self.fileList),
    ...
    def makeTLFiles(self):
        ...
        d.close()
        if self.verbose:
            print("Got {} timelapse images from {} files with {} errors".format(len(self.tlFiles), len(self.fileList),
                                                                                nErrors))
...
class TLMovieMakerConcat(TLMovieMaker):
    def loadMovies(self):
        ...
        self.tlMovies.append(
            TLMovie(self.tlFiles, self.moviesRelPath, self.speedup, self.leastImages, self.motion, self.suffix))


class TLMovieMakerHour(TLMovieMaker):
    def __str__(self):
        return "{}: Movies:{}  SpeedUp:{} fileList:{} range={} to {} leastImages:{} motion:{}".format(
            type(self).__name__, len(self.tlMovies), self.speedup, len(self.fileList), self.dayStartTime,
            self.dayEndTime, self.leastImages, self.motion)
    ...
    def loadMovies(self):
        TLMovieMaker.loadMovies(self)
        groupedByTimeTLFiles = self.groupByTime(self.tlFiles)  # ,TimeDelta(1h))

        # @todo s
        # remove out-of-time hours
        for h in groupedByTimeTLFiles:
            if self.verbose:
                print("Loading movie for Hour{} with {} filtered files from {} total files ".format(h, len(
                    groupedByTimeTLFiles[h]), len(self.tlFiles)))

            tlm = TLMovie(groupedByTimeTLFiles[h], self.moviesRelPath, self.speedup, self.leastImages, self.motion,
                          fpsMax=self.fpsMax, verbose=self.verbose)
            self.tlMovies.append(tlm)
            if len(tlm.tlFiles) > 0:
                tlm.customMovieFilename = (datetime.datetime.combine(tlm.tlFiles[0].datetimeTaken.date(),
                                                                     datetime.time(tlm.tlFiles[0].datetimeTaken.hour, 0,
                                                                                   0, 0))).strftime("%Y-%m-%dT%H-00-00")

                # pprint.pprint(groupedTLFiles)

    @staticmethod
    def groupByTime(localTLFiles):
        groupedTLFiles = OrderedDict()
        for dayhour, dayFiles in itertools.groupby(localTLFiles,
                                                   lambda x: datetime.datetime.combine(x.datetimeTaken.date(),
                                                                                       datetime.time(
                                                                                               x.datetimeTaken.hour, 0,
                                                                                               0, 0))):
            print ("Hour {}".format(dayhour))
            groupedTLFiles[dayhour] = []
            for tlf in sorted(dayFiles):
                groupedTLFiles[dayhour].append(tlf)
...
class TLMovieMakerDay(TLMovieMaker):
    def __str__(self):
        return "{}: Movies:{}  SpeedUp:{} fileList:{} range={} to {} daySliceLength:{} leastImages:{} motion:{}".format(
            type(self).__name__, len(self.tlMovies), self.speedup, len(self.fileList), self.dayStartTime,
            self.dayEndTime, self.daySliceLength, self.leastImages, self.motion)
    ...
    def loadMovies(self):
        ...
        for day in groupedByDayTLFiles:
            dayFilesFiltered = self.filterByHour(groupedByDayTLFiles[day], self.dayStartTime, self.dayEndTime,
                                                 self.verbose)
            if self.verbose: print(
                "Loading movie for {} with {} filtered files from {} total files ".format(day, len(dayFilesFiltered),
                                                                                          len(groupedByDayTLFiles[
                                                                                                  day])))
            self.tlMovies.append(
                TLMovie(dayFilesFiltered, self.moviesRelPath, self.speedup, self.leastImages, self.motion,
                        fpsMax=self.fpsMax, verbose=self.verbose))
...
class TLMovieMakerDayhour(TLMovieMakerDay):
    def loadMovies(self):
        ...
        if self.daySliceLength == None:
            self.daySliceLength = (nptime.nptime.from_time(self.dayEndTime) - nptime.nptime.from_time(
                self.dayStartTime)) / len(groupedByDay)
        ...
        for day in sorted(groupedByDay.keys()):
            ...
            if (self.verbose):
                print ("Day {}: {} to {} ({}/{} files)".format(day, startDateTime, endDateTime, len(dayTLFiles),
                                                               len(groupedByDay[day])))
...
class TLMovie:
    ...
    def __init__(self, tlFiles, movieDir=".", speedup=None, leastImages=None, motion=None, suffix="", fpsMax=None,
                 verbose=None):
    ...
    def __str__(self):
        return "TLMovie: filename={} speedup={} fpsVideo={:.1f} br={} djt={} frames={} spfReal={:.1f} leastImages={} motion:{} ".format(
            self.movieFilename(), self.speedup, self.fpsVideo(), self.bitrate, self.disjointThreshold,
            len(self.tlFiles), self.spfReal(), self.leastImages, self.motion)
    ...
    def selectTLFilesToSuitMaxFPS(self):
        if (self.verbose):
            ...
            print("selectTLFilesToSuitMaxFPS: realDuration = %s files = %d" % (
            self.lastDateTime() - self.firstDateTime(), len(self.tlFiles)))
        ...
        if (self.verbose):
            ...
            print("selectTLFilesToSuitMaxFPS: maxFps=%f fps= %f duration = %s files = %d" % (
            self.fpsMax, self.fpsVideo(), self.lastDateTime() - self.firstDateTime(), len(newTLFiles)))
    ...
    def durationVideo(self):
        # Run-around to avoid "TypeError: unsupported operand type(s) for /: 'datetime.timedelta' and 'int'"
        return datetime.timedelta(seconds=self.durationReal().total_seconds() / self.speedup)
    ...
    def write(self):
        ...
        if (not self.force and os.path.isfile(self.moviePathname())):
            if self.verbose:
                print("Not overwriting {}".format(self.moviePathname()))
            return

        self.writeImageList()

        if self.verbose:
            print ("TLMovie.write(): writing {} images to {} durationReal = {}s (={}) durationVideo= {}s (={}) ".format(
                len(self.tlFiles), self.moviePathname(), self.durationReal().total_seconds(), self.durationReal(),
                self.durationVideo().total_seconds(), self.durationVideo()))
        ...
        safeFilenames = "0"  # 0 = disable safe 1 = enable safe
        # windows
        ...
        # r = subprocess.call(["ffmpeg","-y","-f","concat", "-safe",safeFilenames,"-i",self.listFilename(),"-b:v",str(self.bitrate),"-r",str(self.fpsVideo()),self.moviePathname()],stdout=logFile,stderr=logFile)
        # "-preset","slow","-crf","25", for more quality
        thecall = ["ffmpeg", "-y", "-f", "concat", "-safe", safeFilenames, "-i", self.listFilename(), "-vcodec",
                   "libx264", "-r", str(self.fpsVideo()), self.moviePathname()]
        print(thecall)
        r = subprocess.call(thecall, stdout=logFile, stderr=logFile)
        ...
        if r == 0:
            ...
            print("*** FAILED on writing {} images to {}. Logged to {}".format(len(self.tlFiles), self.moviePathname(),
                                                                               self.logFilename()))
    ...
    def writeImageList(self):
        ...
        for tlf in self.tlFiles:
            ...
            f.write(
                "duration " + str(datetime.timedelta(seconds=tlf.durationReal.total_seconds() / self.speedup)) + "\n")
        f.close()
...
class TLFile:
    ...
    def __repr__(self):
        return '{}\t\t{}\t\t{}\t{}'.format(self.datetimeTaken, self.durationReal, self.filename,
                                           self.isFirst if self.isFirst else "    ")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--force", action='store_true', default=False, help="Force overwrite of existing movies")
    ...
    parser.add_argument("--daystarttime", default="7:00", type=str)
    parser.add_argument("--dayendtime", default="17:00", type=str)
