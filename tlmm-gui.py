...
class ImageOrder(Enum):
    ...
    @staticmethod
    def fromStr(name):
        return getattr( ImageOrder, name )
...
class TLMovieMaker:
    ...
    suffix=""
    fpsMax = None
    imageOrder = ImageOrder.BYDATE
    
    def __init__ (self):
        pass
    
    def __str__(self):
        return "{}:: Movies:{}  SpeedUp:{} DaySliceLength:{} fileList:{} motion:{} imageOrder:{}".format(self.__class__.__name__,len(self.tlMovies),self.speedup,self.daySliceLength,len(self.fileList),self.motion,str(self.imageOrder))
    ...
    def makeTLFiles(self):
        ...
        if (self.verbose): print ("ImageOrder: {}".format( ImageOrder.BYNAME))
        globStr = ','.join(self.fileGlob)
        if self.imageOrder == ImageOrder.BYDATE:
        ...
        #else:
        elif self.imageOrder == ImageOrder.BYNAME:
            # Ignore date/times by setting nominal date/time and adding nominal increment
            
            datetimeTaken = datetime.datetime(2000, 1, 1, 0,0)
            nominalIncrement = datetime.timedelta(seconds=1)


            if (self.verbose): print ("self.fileList {}".format(self.fileList.count))
            self.fileList.sort(key=alphanum_key)    # order by name of files TODO multiple folders - need to sort by filename (not pathname)
            for fn in self.fileList:
                if (self.verbose): print ("{}".format(fn))
                tlf = TLFile(fn,datetimeTaken)
                self.tlFiles.append(tlf)
                datetimeTaken = datetimeTaken + nominalIncrement
...
class TLMovieMakerDay(TLMovieMaker):
    def __str__(self):
        return "{}::: Movies:{}  SpeedUp:{} fileList:{} range={} to {} daySliceLength:{} leastImages:{} motion:{}".format(self.__class__.__name__,len(self.tlMovies),self.speedup,len(self.fileList),self.dayStartTime,self.dayEndTime,self.daySliceLength,self.leastImages,self.motion)
...
def main():
    ...
    parser = GooeyParser(description='Make timelapse movies!')
    #parser = argparse.ArgumentParser("Make timelapse movies")
    parser.add_argument("fileGlob",nargs='+',widget='DirChooser',default="*.jpg",help="Files to process, using wildcards.")
    parser.add_argument("--verbose",action='store_true',default=True,help="Display progress text")
    parser.add_argument("--dryrun",action='store_true',default=False,help="Don't actually save a movie")
    parser.add_argument("--imageorder",choices=['BYDATE','BYNAME'],default='date',help="How to order the images")
    parser.add_argument("--moviesRelPath", default="", type=str,help="Save movies to this path, relative to current working directory")
    parser.add_argument("--nocache",action='store_true',default=False,help="Ignore cache of EXIF file dates")
    #parser.add_argument("--slicetype",choices=SliceType.names(),default="day",widget='Dropdown')
    parser.add_argument("--slicetype",choices=['day','hour','dayhour','concat'],default='concat',help="How to put images together")
    parser.add_argument("--speedup",default=300,type=int)
    parser.add_argument("--leastimages",default=100,type=int,help="Only write movies with at least this many images")
    parser.add_argument("--fpsmax", default=None, type=int,help="Limit Frames Per Second")
    ...
    parser.add_argument("--suffix",default="",type=str,help="Append this suffix to the movie name/s")
    ...
    mm.imageOrder = ImageOrder.fromStr(args.imageorder)
